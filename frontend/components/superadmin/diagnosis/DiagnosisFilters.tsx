  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card p-4 rounded-xl border border-border shadow-sm">
      ...
      <div className="flex flex-1 w-full md:w-auto gap-3 items-center">
        ...
        {/* Department Filter - HIDDEN if Scoped */}
        {!isScoped && (
          <select
            value={departmentId || ""}
            onChange={(e) => setDepartmentId(e.target.value || undefined)}
            className="px-3 py-2 text-sm border border-input rounded-lg bg-background focus:ring-1 focus:ring-primary outline-none cursor-pointer max-w-[170px]"
          >
