...
interface ReceptionistActionsMenuProps {
...
export default function ReceptionistActionsMenu({ receptionist, onEdit, onView, onToggleStatus, onResetPassword }: ReceptionistActionsMenuProps) {
  return (
    <DropdownMenu.Root>
        ...
        >
          ...
          <DropdownMenu.Item 
            ...
            className={`flex items-center gap-2 px-2.5 py-2 text-sm rounded-md outline-none cursor-pointer hover:bg-muted ${
              receptionist.status === "Active" ? "text-destructive hover:text-destructive" : "text-success hover:text-success"
            }`}
          >
