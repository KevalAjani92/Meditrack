...
import { X, PlusCircle, Settings2 } from "lucide-react";
...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  treatment: TreatmentType | HospitalTreatment | null;
  mode: "add" | "edit";
  onSave: (
    treatmentId: string,
...
export default function TreatmentFormModal({
  ...
  useEffect(() => {
    if (isOpen && treatment) {
      reset({
        isActive: isAdd ? true : (treatment as HospitalTreatment).isActive,
      });
    }
  }, [isOpen, treatment, isAdd, reset]);

  if (!treatment) return null;

  const onSubmit = (data: { isActive: boolean }) => {
    onSave(
      treatment.treatment_type_id.toString(),
      (treatment as HospitalTreatment).hospital_treatment_id,
      data.isActive,
    );
    onClose();
  };
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] rounded-xl bg-card p-6 shadow-xl border border-border animate-in zoom-in-95">
          <div className="flex justify-between items-center mb-5 border-b border-border pb-3">
            <Dialog.Title className="text-lg font-bold flex items-center gap-2">
              ...
              {isAdd ? "Enable Treatment Type" : "Manage Treatment Settings"}
            </Dialog.Title>
            <button
              onClick={onClose}
          ...
          <div className="space-y-5">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div
                className={`flex items-center justify-between p-4 border rounded-lg transition-colors ${isAdd ? "bg-card border-border" : (treatment as HospitalTreatment).isActive ? "border-success/30 bg-success/5" : "border-border bg-card"}`}
              >
                <div>
                  <label
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isAdd
                      ? "Treatment will be available for billing/procedures."
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button type="submit">
                  {isAdd ? "Enable Treatment" : "Save Changes"}
                </Button>
              </div>
            </form>
          </div>
