...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  procedure: Procedure | HospitalProcedure | null;
...
export default function ProcedureFormModal({
  ...
  useEffect(() => {
    if (isOpen && procedure) {
      reset({
        isActive: isAdd ? true : (procedure as HospitalProcedure).isActive,
        price: isAdd ? 0 : (procedure as HospitalProcedure).price || 0,
      });
    }
  }, [isOpen, procedure, isAdd, reset]);
  ...
  const onSubmit = (data: { isActive: boolean; price: number }) => {
    onSave(
      ...
      (procedure as HospitalProcedure).hospital_procedure_id,
      data.isActive,
      data.price,
    );
    onClose();
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] rounded-xl bg-card p-6 shadow-xl border border-border animate-in zoom-in-95">
          <div className="flex justify-between items-center mb-5 border-b border-border pb-3">
            <Dialog.Title className="text-lg font-bold flex items-center gap-2">
              ...
              {isAdd ? "Enable Procedure" : "Manage Procedure Settings"}
            </Dialog.Title>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="space-y-5">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div
                className={`flex items-center justify-between p-4 border rounded-lg transition-colors ${isAdd ? "bg-card border-border" : (procedure as HospitalProcedure).isActive ? "border-success/30 bg-success/5" : "border-border bg-card"}`}
              >
                <div>
                  <label
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isAdd
                      ? "Procedure will be billable immediately."
                      : "Uncheck to disable billing."}
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button type="submit">
                  {isAdd ? "Enable Procedure" : "Save Changes"}
                </Button>
              </div>
            </form>
          </div>
