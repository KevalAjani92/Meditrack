...
interface Props {
  ...
  onAdd: (diag: Diagnosis) => void;
}

export default function AvailableDiagnosesSection({ 
  diagnoses, search, onSearchChange, departmentFilter, onDepartmentChange, departmentsList, onAdd 
}: Props) {
  return (
    <div className="flex flex-col h-[650px] bg-card border border-border rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 border-b border-border bg-muted/10 space-y-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Select diagnoses from the global registry to enable them for your hospital.</p>
        </div>
        <DiagnosisSearch 
      ...
      <div className="p-5 flex-1 min-h-0 overflow-y-auto custom-scrollbar bg-background/50">
        {diagnoses.length === 0 ? (
          <div className="h-full flex items-center justify-center text-center p-6 text-muted-foreground border-2 border-dashed border-border rounded-xl">
            <p>No available master diagnoses match your criteria.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {diagnoses.map((diag, i) => (
              <motion.div key={diag.diagnosis_id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}>
                <DiagnosisCard diagnosis={diag} type="master" onAction={onAdd} />
              </motion.div>
            ))}
          </div>
        )}
