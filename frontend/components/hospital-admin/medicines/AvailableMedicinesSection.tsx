...
export default function AvailableMedicinesSection({ 
  medicines, search, onSearchChange, typeFilter, onTypeChange, typesList, onAdd 
}: Props) {
  return (
    <div className="flex flex-col h-[650px] bg-card border border-border rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 border-b border-border bg-muted/10 space-y-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Select medicines from the global registry to enable them for your pharmacy.</p>
        </div>
        <MedicineSearch 
          searchValue={search} onSearchChange={onSearchChange} 
          typeValue={typeFilter} onTypeChange={onTypeChange}
          types={typesList} placeholder="Search by name, code or mfg..." 
        />
      </div>
