...
import { X, PlusCircle, Settings2, IndianRupee, Package } from "lucide-react";
...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  medicine: Medicine | HospitalMedicine | null;
  mode: "add" | "edit";
  onSave: (
    medId: number,
...
export default function MedicineFormModal({
  ...
  const {
    ...
    defaultValues: { isActive: true, price: 0, stock: 0 },
  });

  useEffect(() => {
    if (isOpen && medicine) {
      reset({
        isActive: isAdd ? true : (medicine as HospitalMedicine).isActive,
        price: isAdd ? 0 : (medicine as HospitalMedicine).price || 0,
        stock: isAdd ? 0 : (medicine as HospitalMedicine).stock || 0,
      });
    }
  }, [isOpen, medicine, isAdd, reset]);
  ...
  const onSubmit = (data: {
    ...
    onSave(
      ...
      (medicine as HospitalMedicine).hospital_medicine_id,
      data.isActive,
      data.price,
      data.stock,
    );
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] rounded-xl bg-card p-6 shadow-xl border border-border animate-in zoom-in-95">
          <div className="flex justify-between items-center mb-5 border-b border-border pb-3">
            <Dialog.Title className="text-lg font-bold flex items-center gap-2">
              ...
              {isAdd ? "Enable Medicine in Pharmacy" : "Manage Pharmacy Stock"}
            </Dialog.Title>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="space-y-5">
            ...
            <div
              className={`p-4 rounded-lg border border-border ${isAdd ? "bg-muted/10" : "bg-muted/30 opacity-90"}`}
            >
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-border/50 text-sm">
                <p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div
                className={`flex items-center justify-between p-4 border rounded-lg transition-colors ${isAdd ? "bg-card border-border" : (medicine as HospitalMedicine).isActive ? "border-success/30 bg-success/5" : "border-border bg-card"}`}
              >
                <div>
                  <label
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isAdd
                      ? "Medicine will be available for doctor prescriptions."
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-border">
                ...
                <Button type="submit">
                  {isAdd ? "Enable Medicine" : "Save Changes"}
                </Button>
              </div>
            </form>
          </div>
