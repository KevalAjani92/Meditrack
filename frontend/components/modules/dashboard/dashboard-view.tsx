...
export function DashboardView({ allowedRoles }: DashboardViewProps) {
    ...
    useEffect(() => {
        // Simulate initial data fetch/calculation delay
    ...
    const myAppointments = appointments.filter(apt => {
        if (isPatient) return apt.patientid === user?.id || apt.patientName === user?.name; // Mock fallback
        if (isDoctor) return apt.doctorid === user?.id || apt.doctorName === user?.name;
        return true; // Admin/Receptionist see all
    });
    ...
    const myReceipts = receipts.filter(r => {
        ...
        return true; // Doctors usually don't see financial, but Admin/Recep do
    });

    const totalRevenue = myReceipts.reduce((sum, r) => sum + (r.totalamount || 0), 0);
    // const pendingPayments = myReceipts.filter(r => r.status === 'Pending').length;
    ...
    // 2. Revenue Trend (Mock Monthly Data) - Only show for Admins
    // For Patients/Doctors, maybe show Visit History or similar?
    const revenueData = [
        { name: 'Jan', total: 4000 },
        { name: 'Feb', total: 3000 },
        { name: 'Mar', total: 2000 },
                                        ...
                                        {myAppointments.slice(0, 5).map((apt, i) => (
                                            <div key={i} className="flex items-center justify-between p-3 hover:bg-muted/50 rounded-lg transition-colors cursor-pointer group">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-medium leading-none group-hover:text-primary transition-colors">{apt.patientName}</p>
    ...
    return (
        <Card className="hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-primary/10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <Icon className="h-6 w-6 text-primary opacity-70" />
            </CardHeader>
            <CardContent>
